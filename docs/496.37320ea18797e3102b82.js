...
`;class u extends i.w{async init(){this.search=new p,this.manifest=new f,this.recommended=new b,this.state.view=this.search;const{pathname:t}=new URL(window.location),e=t.split("/");console.warn(e),e?.[2]?.startsWith("recommended")&&(this.state.view=this.recommended),this.render(),this.state.view===this.search&&this.search.submit()}get defaultState(){return{}}render(){return this.html`
		...
		`}renderManifestButton(){return"Manifest"===this.state?.view.title?"":n.k9.wire(this,":manifestButton")`
			<button
				onclick=${()=>this.setState({view:this.manifest})}
			>
				Manifest
			</button>
		`}renderRecommendedButton(){return"Recommended"===this.state?.view.title?"":n.k9.wire(this,":manifestButton")`
			<button
				onclick=${()=>this.navigatePage(this.recommended)}
			>
				Recommended
			</button>
		`}renderUgcButton(){return"Search"===this.state?.view.title?"":n.k9.wire(this,":manifestButton")`
			<button
				onclick=${()=>this.navigatePage(this.search)}
			>
				UGC
			</button>
		`}get view(){return this.state.view?.render?.()??"..."}navigatePage(t){this.setState({view:t}),t.init()}}const g=new class{get assetKinds(){return this._assetKindMap??=new Map([["Unknown","Unknown"],["Film","Film"],["Map","Map"],["Playlist","Playlist"],["Prefab","Prefab"],["TestAsset","Test Asset"],["UgcGameVariant","Mode Variant"],["MapModePair","Map-Mode Pair"],["Project","Project"],["Manifest","Manifest"],["EngineGameVariant","Engine Game Variant"]])}get assetKindArray(){return this._assetKindArray??=[...this.assetKinds.keys()]}getAssetKindByIndex(t){return this.assetKindArray?.[t]??"???"}getAssetKindIndexByName(t){return this.assetKindArray?.indexOf(`${t}`)??2}getAssetStorageTypeByIndex(t){return 2===t?"maps":4===t?"prefabs":6===t?"ugcGameVariants":void 0}get assetSorts(){return this._assetSorts??=new Map([["Name","Name"],["Likes","Likes"],["PlaysRecent","Plays, Recent"],["PlaysAllTime","Plays, All Time"],["NumberOfObjects","Number Of Objects"],["NumberOfRatings","Number Of Ratings"],["ParentAssetCount","Parent Asset Count"],["AverageRating","Average Rating"],["DateCreatedUtc","Date Created"],["DateModifiedUtc","Date Modified"],["DatePublishedUtc","Date Published"],["Bookmarks","Bookmarks"]])}get assets(){return this._assets??=new Map}registerAsset(t,e){this.assets.set(t,e)}async getAsset({kind:t,id:e,version:s}){const i=`${e}/${s}`;if(this.assets.has(i))return this.assets.get(i);const r=await this.fetchAsset({kind:t,id:e,version:s});return r?new v({asset:r}):void 0}async fetchAsset({kind:t,id:e,version:s}){try{const i=new URL(`/api/${this.getAssetStorageTypeByIndex(t)}/${e}/${s}`,`https://${window.location.host.replace("beta.","")}`),r=await fetch(i,{headers:{Accept:"application/json"}});if(!r||!r.ok)throw console.error("[DiscoveryManager.fetchAsset] Failed to fetch",r),new Error(r.status);const n=await r.json();console.log("res!",n)}catch(t){console.error("[DiscoveryManager.fetchAsset]",t)}}get host(){return this._host??"blobs-infiniteugc.svc.halowaypoint.com"}};class p extends i.w{constructor(){super(),this.resultsView=new m}get title(){return"Search"}get defaultState(){return{maxPageSize:24,pageSize:24,page:0,estimatedTotal:0}}init(){console.log("init",this.title),history.pushState(null,null,"/discovery"),this.resultsView.state.results.length||this.submit()}render(){return this.html`
		...
		`}renderSearchOptions(){return n.k9.wire(this,":search")`
		...
		`}renderPageControls(t=upper){return this.pages<2?"":n.k9.wire(this,`:${t}`)`
			<div class="page-controls_wrapper">
				<div class="page-loading">${"upper"===t&&this.state.loading?n.GU.cloneNode(!0):""}</div>
				<button
		...
		`}displayDetails(){d.displayAsset(this)}get id(){return this.asset.AssetId}get version(){return this.asset?.AssetVersionId??this.asset?.VersionId}get cylixURI(){return`ugc/${this.cylixUGCKind}/${this?.id??"0000"}/${this?.version??"0000"}`}get cylixUGCKind(){const t=this.assetKindIndex;return 2===t?"map":4===t?"prefab":6===t?"mode":"asset"}get waypointURI(){return`/hi/${g.getAssetStorageTypeByIndex(this.assetKindIndex)}/${this?.id}/versions/${this?.version}`}get waypointBrowserURI(){return`/halo-infinite/ugc/${this.cylixUGCKind}s/${this?.id}`}get title(){return this.asset?.Name??this.asset?.PublicName??"???"}get description(){return this.asset?.Description??"..."}get originalAuthor(){return this.asset?.OriginalAuthor??this.asset?.Admin??"???"}get contributors(){return[...new Set(this.asset?.Contributors||[])]}get assetKind(){return g.getAssetKindByIndex(this.asset?.AssetKind??this.asset?.AssetHome)}get assetKindIndex(){return this.asset?.AssetKind??this.asset?.AssetHome??0}get defaultThumbnail(){return"https://hi.cylix.guide/ugc/images/default_sm.jpg"}get defaultImage(){return"https://hi.cylix.guide/ugc/images/default.jpg"}get thumbnail(){return this.asset?.ThumbnailUrl??`${this.getAssetRelativeUrl("images/thumbnail.jpg")}`}get hero(){return this.getImageBlob("hero")}get screenshot(){return this.getImageBlob("screenshot1")}get mainImage(){return 2===this.assetKindIndex||4===this.assetKindIndex?this.hero:6===this.assetKindIndex?this.screenshot:this.thumbnail}getImageBlob(t="thumbnail"){return`https://${g.host}/ugcstorage/${this.cylixUGCKind}/${this.id}/${this.version}/images/${t}.jpg`}get averageRating(){return parseFloat(this.asset?.AverageRating??this.asset?.AssetStats?.AverageRating??0).toFixed(2)}get bookmarks(){return this.likes||parseInt(this.asset.Bookmarks??this.asset?.AssetStats?.Favorites??0)}get likes(){return parseInt(this.asset?.Likes??this.asset?.AssetStats?.Favorites??0)}get playsAllTime(){return parseInt(this.asset?.PlaysAllTime??this.asset?.AssetStats?.PlaysAllTime??0)}get playsRecent(){return parseInt(this.asset?.PlaysRecent??this.asset?.AssetStats?.PlaysRecent??0)}get tags(){return[...new Set(this.asset?.Tags||[])]}get objectCount(){return parseInt(this.asset?.NumberOfObjects||0)}get is343(){return this._is343??=this.asset?.OriginalAuthor?.includes("[343]")||this.asset?.OriginalAuthor?.startsWith("2")}get isRecommended(){return this._isRecommended??=(this.asset?.ParentAssetCount??this.asset?.AssetStats?.ParentAssetCount>0)&&!this.is343}get dateModified(){const t=this.asset?.DateModifiedUtc?.ISO8601Date;return t&&Date.parse(t)?new Date(t):new Date("2021/11/15")}get dateCreated(){const t=this.asset?.DateCreatedUtc?.ISO8601Date;return t&&Date.parse(t)?new Date(t):new Date("2021/11/15")}get datePublished(){const t=this.asset?.DatePublishedUtc?.ISO8601Date;return t&&Date.parse(t)?new Date(t):new Date("2021/11/15")}get lastModifiedVersion(){return this.asset?.VersionNumber?`Version ${this.asset.VersionNumber}`:this.dateModified.toLocaleDateString(void 0,{year:"numeric",month:"long",day:"numeric"})}get hasNodeGraph(){return!0===this.asset.HasNodeGraph}get isCopyProtected(){return!0===this.asset.ReadOnlyClones}getAssetRelativeUrl(t){const e=this.asset?.Files?.Prefix;return`${e}${t}`}}class f extends p{constructor(){super(),this.resultsView=new y}get title(){return"Manifest"}renderSearchOptions(){return this.manifest||this.loadManifest(),n.k9.wire(this,":search")`
		<div class="ugc_search-options">
			<div class="option">
				<button 
					class="hi-box"
					onclick=${()=>a.v.showView(this.renderLoadModal())}
				>
					Load Manifest
				</button>
			</div>
		...
		`}renderLoadModal(){return n.k9.wire(this,":loadModal")`
		...
		`}}class y extends i.w{init(t){this.response=t,t?.length&&(this.state.results=this.response),this.render()}get defaultState(){return{pageSize:24,results:[]}}render(){return console.log("ManifestView.rend",this.state.results),this.html`
			<div class="ugc_results_wrapper">
				<ul class="ugc_results">
					${this.currentPage.map((t=>n.k9.wire()`<li>
						${{any:t.render("results"),placeholder:h.cloneNode(!0)}}
					</li>`))}
				</ul>
			</div>
		`}get currentPage(){return this.state.results.map((t=>new v({asset:t})))}}}}]);